...
module PlayerInput
  ...
  def insert_computer_move
    ...
    puts "\nThe computer moved their #{player2_piece.piece_name} from #{Generic.raw_to_string(player2.selected_grid.position)} to #{player2_piece.current_location}.\n".green
  end
  ...
  def convert(str)
    ...
    if str_length == 2 && str[0].length == 1 && str[1].length == 1
      ...
      #sleep 2
      show_board("#{current_player.name} You have input the wrong amount of characters, please only input 2!")
      position
    end
  end
  ...
  def validate_input(str)
    ...
    if /[1-8][a-h]|[a-h][1-8]/.match(str)
      ...
      show_board("#{current_player.name} You have input the incorrect characters, please only input A-H and 1-8!")
      position
    end
  end
